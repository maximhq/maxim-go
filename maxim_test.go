...
func TestMaximSDKOutOfOrderMessages(t *testing.T) {
	...
	span1Id := uuid()
	logger.AddSpanToTrace(trace.Id(), &logging.SpanConfig{Id: span1Id, Name: StrPtr("Test Span")})
	...
	retrievalId := uuid()
	logger.AddRetrievalToSpan(span1Id, &logging.RetrievalConfig{Id: retrievalId, Name: StrPtr("Test Retrieval")})
